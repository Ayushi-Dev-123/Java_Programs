class AddPolynomial{
   ...
   public static class Node{
     ...
     int power;
     Node next;
     public Node(int cofficient, int power){
   ...
   public void create(int cofficient,int power){
     Node node = new Node(cofficient,power);
     if(head == null){
       head = node;
     }
     else{
     ...
     public void add(AddPolynomial eq1, AddPolynomial eq2, int length){
        ...
        while(i<=length){
          ...
          if(eq1.head.power == eq2.head.power){
            node.cofficient = eq1.head.cofficient + eq2.head.cofficient;
            node.power = eq1.head.power;
            eq1.head = eq1.head.next;
            eq2.head = eq2.head.next;
          }
          else if(eq1.head.power > eq2.head.power){
            node.cofficient = eq1.head.cofficient;
            node.power = eq1.head.power;
            eq1.head = eq1.head.next;
          }
          else{
            node.cofficient = eq2.head.cofficient;
   ...
   public void traverse(){
     Node temp = head;
     while(temp!=null){
        System.out.print(temp.cofficient+"x^"+temp.power+" ");
        temp = temp.next;
     }
     System.out.println();
   }
   public static void main(String arg[]){
         ...
         AddPolynomial resultantEq = new AddPolynomial(); 
         resultantEq.addPolynomial(eq1,eq2);
                 
         resultantEq.traverse();    
